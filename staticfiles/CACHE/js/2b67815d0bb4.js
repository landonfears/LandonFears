...
angular.forEach(element.find('.skill-bar'),function(skill,key){$(skill).stop().animate({width:0},'slow');});$(this).width(element.parent().width());});}
function endAffix(){$(window).off('.affix');element.removeClass("affix affix-top affix-bottom").removeData("bs.affix");element.css('opacity',1);element.width(element.parent().width());element.css('top',0);angular.forEach(element.find('.skill-bar'),function(skill,key){$(skill).width($(skill).data('level')+'%');});}},templateUrl:'static/templates/products/single.product.html'};return directive;}})();(function(){'use strict';angular.module('salesfiction.products.directives').directive('job',job);function job($timeout,$sce){var directive={restrict:'E',replace:true,transclude:true,scope:{company:'@',jobTitle:'@',jobDate:'@',jobLocation:'@',jobItems:'=',jobItemIcon:'@'},link:function(scope,element,attrs){if(!scope.jobItemIcon)scope.jobItemIcon='caret-right';},templateUrl:'static/templates/products/job.product.html'};return directive;}})();(function(){'use strict';angular.module('salesfiction.products.directives').directive('skill',skill);function skill($timeout,$sce){var directive={restrict:'E',replace:true,transclude:true,scope:{name:'@',level:'@',levelName:'@',skillIcon:'@'},link:function(scope,element,attrs){if(!scope.skillIcon)scope.skillIcon='check';if(!canAffix()){$(window).off('.affix');element.closest('.product').removeClass("affix affix-top affix-bottom").removeData("bs.affix");element.closest('.product').css('opacity',1);element.closest('.product').width('100%');element.closest('.product').css('top',0);var skill=element.find('.skill-bar');skill.width(scope.level+'%');}
function canAffix(){return $('.affix-breakpoint').is(':visible')&&!$.browser.mobile;}},templateUrl:'static/templates/products/skill.product.html'};return directive;}})();(function(){'use strict';angular.module('salesfiction.products.directives').directive('about',about);function about($timeout){var directive={restrict:'E',replace:true,transclude:true,scope:{name:'@',aboutItems:'=',aboutItemIcon:'@',githubUrl:'@',linkedinUrl:'@'},link:function(scope,element,attrs){if(!scope.aboutItemIcon)scope.aboutItemIcon='caret-right';scope.social=[];if(scope.githubUrl){scope.social.push({url:scope.githubUrl,icon:'github'});}
if(scope.linkedinUrl){scope.social.push({url:scope.linkedinUrl,icon:'linkedin'});}
scope.socialNum=Math.floor(12/scope.social.length);},templateUrl:'static/templates/products/about.product.html'};return directive;}})();