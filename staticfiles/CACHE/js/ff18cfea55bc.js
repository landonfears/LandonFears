...
bindJQuery();publishExternalAPI(angular);jqLite(document).ready(function(){angularInit(document,bootstrap);});})(window,document);!window.angular.$$csp()&&window.angular.element(document).find('head').prepend('<style type="text/css">@charset "UTF-8";[ng\\:cloak],[ng-cloak],[data-ng-cloak],[x-ng-cloak],.ng-cloak,.x-ng-cloak,.ng-hide:not(.ng-hide-animate){display:none !important;}ng\\:form{display:block;}</style>');(function(window,angular,undefined){'use strict';var $resourceMinErr=angular.$$minErr('$resource');var MEMBER_NAME_REGEX=/^(\.[a-zA-Z_$][0-9a-zA-Z_$]*)+$/;function isValidDottedPath(path){return(path!=null&&path!==''&&path!=='hasOwnProperty'&&MEMBER_NAME_REGEX.test('.'+path));}
...
var isInstanceCall=this instanceof Resource;var value=isInstanceCall?data:(action.isArray?[]:new Resource(data));var httpConfig={};var responseInterceptor=action.interceptor&&action.interceptor.response||defaultResponseInterceptor;var responseErrorInterceptor=action.interceptor&&action.interceptor.responseError||undefined;forEach(action,function(value,key){if(key!='params'&&key!='isArray'&&key!='interceptor'){httpConfig[key]=copy(value);}});if(hasBody)httpConfig.data=data;route.setUrlParams(httpConfig,extend({},extractParams(data,action.params||{}),params),action.url);var promise=$http(httpConfig).then(function(response){var data=response.data,promise=value.$promise;if(data){if(angular.isArray(data)!==(!!action.isArray)){throw $resourceMinErr('badcfg','Error in resource configuration for action `{0}`. Expected response to '+'contain an {1} but got an {2}',name,action.isArray?'array':'object',angular.isArray(data)?'array':'object');}
...
updated=true;}}}}}]).factory('$cookieStore',['$cookies',function($cookies){return{get:function(key){var value=$cookies[key];return value?angular.fromJson(value):value;},put:function(key,value){$cookies[key]=angular.toJson(value);},remove:function(key){delete $cookies[key];}};}]);})(window,window.angular);(function(){'use strict';angular.module('salesfiction',['salesfiction.config','salesfiction.routes','salesfiction.layout','salesfiction.stories','salesfiction.scripts','salesfiction.products']);angular.module('salesfiction.config',[]);angular.module('salesfiction.routes',['ngRoute']);angular.module('salesfiction').run(run);run.$inject=['$http'];function run($http){$http.defaults.xsrfHeaderName='X-CSRFToken';$http.defaults.xsrfCookieName='csrftoken';}})();(function(){'use strict';angular.module('salesfiction.config').config(config);config.$inject=['$locationProvider'];function config($locationProvider){$locationProvider.html5Mode(true);$locationProvider.hashPrefix('!');}})();(function(){'use strict';angular.module('salesfiction.routes').config(config);config.$inject=['$routeProvider'];function config($routeProvider){$routeProvider.when('/',{controller:'IndexController',templateUrl:'/static/templates/layout/index.html'}).otherwise('/');}})();(function(){'use strict';angular.module('salesfiction.layout',['salesfiction.layout.controllers','salesfiction.layout.directives']);angular.module('salesfiction.layout.controllers',[]);angular.module('salesfiction.layout.directives',[]);})();(function(){'use strict';angular.module('salesfiction.layout.controllers').controller('IndexController',IndexController);IndexController.$inject=['$scope','Stories','Scripts'];function IndexController($scope,Stories,Scripts){Stories.latest().then(storiesSuccess);function storiesSuccess(data,status,headers,config){$scope.stories=data.data;if($scope.stories.length){angular.forEach($scope.stories[0].scripts,function(script,index){Scripts.get(script.id).then(function(scriptData){$scope.stories[0].scripts[index].script=scriptData.data;});});}}}})();(function(){'use strict';angular.module('salesfiction.stories',['salesfiction.stories.services']);angular.module('salesfiction.stories.services',[]);})();(function(){'use strict';angular.module('salesfiction.stories.services').factory('Stories',Stories);Stories.$inject=['$http'];function Stories($http){var Stories={latest:latest};return Stories;function latest(){return $http.get('/api/lateststory/');}}})();(function(){'use strict';angular.module('salesfiction.scripts',['salesfiction.scripts.directives','salesfiction.scripts.services']);angular.module('salesfiction.scripts.directives',[]);angular.module('salesfiction.scripts.services',[]);})();(function(){'use strict';angular.module('salesfiction.scripts.services').factory('Scripts',Scripts);Scripts.$inject=['$http'];function Scripts($http){var Scripts={get:get,prettyUrl:prettyUrl};return Scripts;function get(id){return $http.get('/api/script/'+id+'/');}
function prettyUrl(name){return name.toLowerCase().replace(/ /g,'-').replace(/[^\w-]+/g,'');}}})();(function(){'use strict';angular.module('salesfiction.layout.directives').directive('dropdownScripts',dropdownScripts);dropdownScripts.$inject=['Scripts','$timeout','$location'];function dropdownScripts(Scripts,$timeout,$location){var directive={restrict:'E',replace:true,scope:{scripts:'='},link:function(scope,element,attrs){scope.prettyUrl=Scripts.prettyUrl;scope.activateScriptLink=function(event){$('.dropdown-menu li.active').removeClass('active');$(event.target).parent().addClass('active');$('html, body').animate({scrollTop:$('#'+$(event.target).attr('href')).offset().top},700);};var path=$location.url().replace('/','');scope.$watch('scripts[0].script',function(newval,oldval){$timeout(function(){if(newval&&path&&$('#'+path).length){$('html, body').animate({scrollTop:$('#'+path).offset().top},1000);}},500);});},templateUrl:'static/templates/layout/dropdown.scripts.html'};return directive;}})();(function(){'use strict';angular.module('salesfiction.scripts.directives').directive('scripts',scripts);function scripts(){var directive={restrict:'E',replace:true,scope:{scripts:'='},templateUrl:'static/templates/scripts/scripts.html'};return directive;}})();(function(){'use strict';angular.module('salesfiction.scripts.directives').directive('singleScript',singleScript);singleScript.$inject=['Scripts','$timeout'];function singleScript(Scripts,$timeout){var directive={restrict:'E',replace:true,scope:{script:'=',even:'='},link:function(scope,element,attrs){$timeout(function(){scope.slug=Scripts.prettyUrl(scope.script.script.script_name);},500);},templateUrl:'static/templates/scripts/single.script.html'};return directive;}})();(function(){'use strict';angular.module('salesfiction.products',['salesfiction.products.directives',]);angular.module('salesfiction.products.directives',[]);})();(function(){'use strict';angular.module('salesfiction.products.directives').directive('singleProduct',singleProduct);function singleProduct($timeout,$location,$compile){var directive={restrict:'E',replace:true,scope:{product:'='},link:function(scope,element,attrs){$timeout(function(){var el=$compile(angular.element(scope.product.product_content))(scope);el.insertAfter(element.find('h1'));checkAffix();$(window).on("resize",function(){checkAffix();});},4000);scope.isAffix=!canAffix();function checkAffix(){var affix=canAffix();if(!affix&&scope.isAffix){scope.isAffix=false;endAffix();}
else if(affix&&!scope.isAffix){scope.isAffix=true;startAffix();}}
function canAffix(){return $('.affix-breakpoint').is(':visible')&&!$.browser.mobile;}
function startAffix(){element.affix({offset:{top:function(e){var scriptContent=e.closest('.script-content');var prevScript=e.closest('.script-fullwidth').prev('.script-fullwidth').find('.script-content');return(this.top=scriptContent.offset().top-76);},bottom:function(e){var nextScript=e.closest('.script-fullwidth').next('.script-fullwidth').find('.script-content');var bottom=(nextScript.length===0)?0:$(document).height()-nextScript.offset().top;return(this.bottom=bottom);}}}).on('affix.bs.affix',function(e){element.stop().animate({opacity:'1'},'slow');var nextScript=element.closest('.script-fullwidth').next('.script-fullwidth').find('.script-content');if(nextScript.length){angular.forEach(nextScript.find('.skill-bar'),function(skill,key){$(skill).stop().animate({width:0},'slow');});}}).on('affixed.bs.affix',function(e){var index=element.closest('.script-fullwidth').index(),firstScript=$('.script-fullwidth').eq(0).find('.script-content');$('.dropdown-menu li.active').removeClass('active');$('.dropdown-menu li').eq(index).addClass('active');scope.$apply(function(){$location.url('/'+$('.dropdown-menu li').eq(index).find('a').attr('href'));});angular.forEach(element.find('.skill-bar'),function(skill,key){$(skill).stop().animate({width:$(skill).data('level')+'%'},'slow');});if(index>0){$(this).css({'top':firstScript.offset().top-15,});}
$(this).width(element.parent().width());}).on('affixed-top.bs.affix',function(e){var index=element.closest('.script-fullwidth').index(),firstScript=$('.script-fullwidth').eq(0).find('.script-content');var prevScript=element.closest('.script-fullwidth').prev('.script-fullwidth').find('.script-content');if(prevScript.length){prevScript.find('.product').stop().animate({opacity:'1'},'slow');}
else{scope.$apply(function(){$location.url('/');});}
if(index>0){$(this).css({'top':0});}
$(this).width(element.parent().width());}).on('affixed-bottom.bs.affix',function(e){var nextScript=element.closest('.script-fullwidth').next('.script-fullwidth').find('.script-content');if(nextScript.length){element.stop().animate({opacity:'.2'},'slow');}
angular.forEach(element.find('.skill-bar'),function(skill,key){$(skill).stop().animate({width:0},'slow');});});}
function endAffix(){$(window).off('.affix');element.removeClass("affix affix-top affix-bottom").removeData("bs.affix");element.css('opacity',1);element.width('100%');element.css('top',0);angular.forEach(element.find('.skill-bar'),function(skill,key){$(skill).width($(skill).data('level')+'%');});}},templateUrl:'static/templates/products/single.product.html'};return directive;}})();(function(){'use strict';angular.module('salesfiction.products.directives').directive('job',job);function job($timeout,$sce){var directive={restrict:'E',replace:true,transclude:true,scope:{company:'@',jobTitle:'@',jobDate:'@',jobLocation:'@',jobItems:'=',jobItemIcon:'@'},link:function(scope,element,attrs){if(!scope.jobItemIcon)scope.jobItemIcon='caret-right';},templateUrl:'static/templates/products/job.product.html'};return directive;}})();(function(){'use strict';angular.module('salesfiction.products.directives').directive('skill',skill);function skill($timeout,$sce){var directive={restrict:'E',replace:true,transclude:true,scope:{name:'@',level:'@',levelName:'@',skillIcon:'@'},link:function(scope,element,attrs){if(!scope.skillIcon)scope.skillIcon='check';if(!canAffix()){$(window).off('.affix');element.closest('.product').removeClass("affix affix-top affix-bottom").removeData("bs.affix");element.closest('.product').css('opacity',1);element.closest('.product').width('100%');element.closest('.product').css('top',0);var skill=element.find('.skill-bar');skill.width(scope.level+'%');}
function canAffix(){return $('.affix-breakpoint').is(':visible')&&!$.browser.mobile;}},templateUrl:'static/templates/products/skill.product.html'};return directive;}})();(function(){'use strict';angular.module('salesfiction.products.directives').directive('about',about);function about($timeout){var directive={restrict:'E',replace:true,transclude:true,scope:{name:'@',aboutItems:'=',aboutItemIcon:'@'},link:function(scope,element,attrs){if(!scope.aboutItemIcon)scope.aboutItemIcon='caret-right';},templateUrl:'static/templates/products/about.product.html'};return directive;}})();(function(){'use strict';angular.module('salesfiction.products.directives').directive('social',social);function social($timeout){var directive={restrict:'E',replace:true,transclude:true,scope:{githubUrl:'@',linkedinUrl:'@'},link:function(scope,element,attrs){scope.social=[];if(scope.githubUrl){scope.social.push({url:scope.githubUrl,icon:'github'});}
if(scope.linkedinUrl){scope.social.push({url:scope.linkedinUrl,icon:'linkedin'});}
scope.socialNum=Math.floor(12/scope.social.length);},templateUrl:'static/templates/products/social.product.html'};return directive;}})();